...
public class ProductoExcelExportar {
	//referencia al libro
		...
		public ProductoExcelExportar(List<Producto> producto) {
		...
		private void writeHeaderLine() {
			...
			//propiedades qeu queremos mostrar en el excel (encabezados)
			crearCell(row,0,"ID",style);
			crearCell(row,1,"NOMBRE",style);
			crearCell(row,2,"Precio",style);
			crearCell(row,3,"CANTIDAD",style);
			crearCell(row,4,"Categoría",style);
		}
		...
		private void writeDataLines() {
			...
			for(Producto re: producto) {
				...
				crearCell(row,columCount++,String.valueOf(re.getPrecio()),style);
				crearCell(row,columCount++,String.valueOf(re.getCantidad()),style);
				crearCell(row,columCount++,re.getCategoria().getNombre(),style);
